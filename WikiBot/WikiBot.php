...
//ob_start(); //gotta have this
  ...
  class Wikibot {
      ...
      public function get_users($wiki = "")//get list users
      {
		//fb($wiki);
          $response = $this->callAPI($wiki, 'api.php?action=query&list=allusers&aulimit=100');
			fb($response);
          return ($response);
      }
      ...
      private function callAPI($wiki, $url, $format = "php") {
          $wiki = $this->wiki($wiki);
          $ch = curl_init();
		$fullUrl=$wiki . $url . "&maxlag=" . $this->max_lag . "&format=$format";
		fb($fullUrl);
